...
def generate_questions(question_template: str, relation: str, bounds: List[Tuple[int, int]], n_sig_figs: List[int], num_questions: int) -> List[Tuple[str, str]]:
    ...
    for _ in range(num_questions):
        var_values = {var: sigfig(sigfig(str(np.random.choice(np.logspace(np.log10(low), np.log10(high)))), sig_figs=nsf).scientific_notation()) for var, (low, high), nsf in zip(variables, bounds, n_sig_figs)}
...
def main():
    ...
    for variable in variables:
        ...
        nsf = int(input("Number of significant figures: "))
        n_sig_figs.append(nsf)

    questions = generate_questions(question_template, relation, bounds, n_sig_figs, 100)

    basename = "sigfigaddition" # str(input("base name of csv (omit .csv extension): "))
