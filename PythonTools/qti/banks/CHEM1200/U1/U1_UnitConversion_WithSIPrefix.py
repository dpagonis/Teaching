...
def generate_question():
    ...
    question_options = [
        'Convert {v1_str} {prefix1}{u1} to {prefix2}{u2}. There are {conv_factor} {usmall} in one {ularge}.;answer',
    ]
    ...
    while True: #make sure answer isn't below 1e-4 for numerical tolerance question
        type = random.randint(1,4)
        if type <=3:
            prefix1 = ''
            prefix2 = random.choice(prefix_pool)
        elif type == 4:
            prefix1 = random.choice(prefix_pool)
            prefix2 = random.choice(prefix_pool)
        ...
        v1 = sf.random_value((1e-3,1e3),(3,5),value_log=True,units_str=(prefix1+u1))
        v2 = v1.convert_to((prefix2+u2))
        ...
        if conv_factor.sig_figs > 5:
            print(conv_factor)
            conv_factor = sf(conv_factor.as_num,sig_figs=5)
