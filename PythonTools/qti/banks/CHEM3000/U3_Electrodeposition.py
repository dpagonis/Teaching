...
def generate_question():
    ...
    question_options = [
        'Calculate the number of moles of {red_form} formed by flowing a current of {I} A into a solution for {t_min} minutes.<br>{r_html};mol.answers(sf_tolerance=1)'
    ]

    # generating random values for variables, doing calculations, & prepping namespace here
    
    row = reactions.sample(1).iloc[0]
    ...
    r_html = create_mattext_element(r.tex)

    F=96486
    I = sf.random_value((0.01,10),(2,4))
    t_min = sf.random_value((1,30),(2,3),units_str='min')

    mol = I * t_min.convert_to('s') * red_form.coefficient / (n_e * F)
