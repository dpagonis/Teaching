...
class uncertainvalue:
    ...
    @staticmethod
    def random_value(mean_range=(1, 100), stdev_range=(0.1, 10), n_range=(3, 20), mean_log=False, stdev_log=False, stdev_relative=False,no_hidden_digits=False):
        ...
        if(no_hidden_digits):
            uv = uncertainvalue(mean, stdev, n)
            mean = float(uv.mean.scientific_notation())
            stdev = float(uv.stdev.scientific_notation())
...
def main():
    x = uncertainvalue.random_value(no_hidden_digits=True)
    print(x)
    print(x.mean.scientific_notation())
    print(x.stdev.scientific_notation())
