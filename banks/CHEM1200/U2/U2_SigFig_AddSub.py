...
def generate_question():
    ...
    while True:
        ...
        if random.random() < 0.7:
            x = sf.random_value((1e-3,1e4),(1,5),True)
            y = sf.random_value((x.value/50,x.value*50),(1,5),True)
        else:
            x = sf.random_value((1e-12,1e12),(1,5),True)
            y = sf.random_value((x.value/50,x.value*50),(1,5),True)
        ...
        if operator == 'add':
            ...
            if y.value > x.value: #always positive values
                holder = x
                x = y
                y = x
            answer = x-y
